...
class ProductRepository extends BaseRepository {
  async getAll({ search, category, minPrice, maxPrice, inStock, manufacturerId, sort = 'created_at', order = 'desc', page = 1, limit = 20 } = {}) {
  ...
  async getByIdForManufacturer(productId, manufacturerId) {
  ...
  async updateForManufacturer(productId, manufacturerId, updates) {
    const { data, error } = await supabase
      .from('products')
      .update(updates)
      .eq('id', productId)
  ...
  async deleteForManufacturer(productId, manufacturerId) {
    const { data, error } = await supabase
      .from('products')
      .delete()
      .eq('id', productId)
